...
function Move-PSFPSItemsToRecycleBin {
	...
	process {
		...
		Get-ChildItem -Path $RootPath -Recurse | Where-Object { ($_.HasChildren -eq $false) -and ($FolderTemplateIds -notcontains $_.TemplateId) -and ($IdsToSkip -notcontains $_.ID.ToString()) -and ($linkDB.GetReferrers($_).Count -eq 0) } | ForEach-Object {
			$Item = $_
